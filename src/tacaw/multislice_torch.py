"""
PyTorch-accelerated multislice calculation functions
...
class ProbeTorch:
    ...
    def __init__(self, xs, ys, mrad, eV, device=None):
        ...
        self.kxs = torch.fft.fftfreq(nx, d=dx, device=device)
        self.kys = torch.fft.fftfreq(ny, d=dy, device=device)
        ...
        if mrad == 0:
            self.array = torch.ones((nx, ny), dtype=complex_dtype, device=device)
        else:
            reciprocal = torch.zeros((nx, ny), dtype=complex_dtype, device=device)
            radius = (mrad * 1e-3) / self.wavelength  # Convert mrad to reciprocal space units
            
            kx_grid, ky_grid = torch.meshgrid(self.kxs, self.kys, indexing='ij')
            radii = torch.sqrt(kx_grid**2 + ky_grid**2)
            
            mask = radii < radius
            reciprocal[mask] = 1.0
            
            self.array = torch.fft.ifftshift(torch.fft.ifft2(reciprocal))
