...
options = { "ADF":[False,True], "loop_probes":[False,10], "use_memmap":[False,True], "prism":[False,25], "return_layers":[-1,None], "min_dk":[0.0,.1], "kth":[1,3], "skip_vacuum":[False,True] }
...
for n,kwargs in enumerate(kwargCombos):
    ...
    for i in range(2):
        if not kwargs.get("ADF",False) and kwargs.get("return_layers",-1) in (None,[]): # skip nonsense combo (since ADF-in-post requires wavefunction_data returned)
            continue
        if n<start or n>end:
            continue
        #os.system("rm -rf psi_data")
